...
static dblue::Logger::ptr g_logger = DBLUE_LOG_NAME("system");
...
HttpConnection::~HttpConnection() {
    DBLUE_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    //         DBLUE_LOG_INFO(g_logger) << "content_len=" << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = dblue::GetCurrentMS();
    ...
    if (!ptr) {
        ...
        if (!addr) {
            DBLUE_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if (!sock) {
            DBLUE_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if (!sock->connect(addr)) {
            DBLUE_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection *ptr, HttpConnectionPool *pool) {
    ++ptr->m_request;
    if (!ptr->isConnected() || ((ptr->m_createTime + pool->m_maxAliveTime) >= dblue::GetCurrentMS()) || (ptr->m_request >= pool->m_maxRequest)) {
        delete ptr;
        --pool->m_total;
        return;
    }
