...
void test()
{
     ...
     */
    DEEPBLUE_LOG_INFO(g_logger) << "TEST";
...
#define XX(type, len, write_fun, read_fun, base_len)                    \
    {                                                                   \
        ...
        deepblue::ByteArray::ptr ba(new deepblue::ByteArray(base_len)); \
        for (auto &i : vec)                                             \
        {                                                               \
            ba->write_fun(i);                                           \
        }                                                               \
        ...
        {                                                               \
            type v = ba->read_fun();                                    \
            DEEPBLUE_ASSERT(v == vec[i]);                               \
        }                                                               \
        DEEPBLUE_ASSERT(ba->getReadSize() == 0);                        \
        DEEPBLUE_LOG_INFO(g_logger) << #write_fun "/" #read_fun         \
                                                  " (" #type ") len="   \
                                    << len                              \
                                    << " base_len=" << base_len         \
                                    << " size=" << ba->getSize();       \
...
#define XX(type, len, write_fun, read_fun, base_len)                                     \
    {                                                                                    \
        ...
        DEEPBLUE_LOG_INFO(g_logger) << #write_fun "/" #read_fun                          \
                                    ...
                                    << " size=" << ba->getSize();                        \
        ba->setPosition(0);                                                              \
        DEEPBLUE_ASSERT(ba->writeToFile("/tmp/" #type "_" #len "-" #read_fun ".dat"));   \
        deepblue::ByteArray::ptr ba2(new deepblue::ByteArray(base_len * 2));             \
        DEEPBLUE_ASSERT(ba2->readFromFile("/tmp/" #type "_" #len "-" #read_fun ".dat")); \
        ba2->setPosition(0);                                                             \
        DEEPBLUE_ASSERT(ba->toString() == ba2->toString());                              \
        DEEPBLUE_ASSERT(ba->getPosition() == 0);                                         \
        DEEPBLUE_ASSERT(ba2->getPosition() == 0);                                        \
    }
    XX(int8_t, 100, writeFint8, readFint8, 1);
    XX(uint8_t, 100, writeFuint8, readFuint8, 1);
    XX(int16_t, 100, writeFint16, readFint16, 1);
