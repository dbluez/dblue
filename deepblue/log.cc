...
#undef XX
    ...
    void Logger::setFormatter(const std::string &val)
    {
        ...
        deepblue::LogFormatter::ptr new_val(new deepblue::LogFormatter(val));
        if (new_val->isError())
        {
...
#undef XX
    ...
    deepblue::ConfigVar<std::set<LogDefine>>::ptr g_log_defines =
        deepblue::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
    ...
    struct LogIniter
    {
        LogIniter()
        {
            g_log_defines->addListener([](const std::set<LogDefine> &old_value,
                                       ...
                                       {
            DEEPBLUE_LOG_INFO(DEEPBLUE_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                deepblue::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
                    logger = DEEPBLUE_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        //修改的logger
                        logger = DEEPBLUE_LOG_NAME(i.name);
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    //删除logger
                    auto logger = DEEPBLUE_LOG_NAME(i.name);
