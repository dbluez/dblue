...
namespace deepblue
{
    namespace http
    {
        ...
        int32_t NotFoundServlet::handle(deepblue::http::HttpRequest::ptr request, deepblue::http::HttpResponse::ptr response, deepblue::http::HttpSession::ptr session)
        {
            response->setStatus(deepblue::http::HttpStatus::NOT_FOUND);
            response->setHeader("Server", "deepblue/1.0.0");
