...
#undef XX
    ...
    IOManager::FdContext::EventContext &IOManager::FdContext::getEventContext(IOManager::Event event)
    {
        ...
        case IOManager::WRITE:
            return write;
        default:
            DEEPBLUE_ASSERT2(false, "getContext");
        }
    ...
    int IOManager::addEvent(int fd, Event event, std::function<void()> cb)
    {
        ...
        if (DEEPBLUE_UNLIKELY(fd_ctx->events & event))
        {
            DEEPBLUE_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                                         << " event=" << (EPOLL_EVENTS)event
                                         << " fd_ctx.event=" << (EPOLL_EVENTS)fd_ctx->events;
            DEEPBLUE_ASSERT(!(fd_ctx->events & event));
        }
        ...
        if (rt)
        {
            DEEPBLUE_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
        ...
        DEEPBLUE_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
        ...
        else
        {
            ...
            DEEPBLUE_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
        }
        return 0;
    }
    ...
    bool IOManager::cancelAll(int fd)
    {
        ...
        DEEPBLUE_ASSERT(fd_ctx->events == 0);
        return true;
    }
    ...
    {
        // DEEPBLUE_LOG_DEBUG(g_logger) << "tickle";
        if (!hasIdleThreads())
        {
            return;
        }
        int rt = write(m_tickleFds[1], "T", 1);
        DEEPBLUE_ASSERT(rt == 1);
    }
    ...
    void IOManager::idle()
    {
        ...
        while (true)
        {
            ...
            if (DEEPBLUE_UNLIKELY(stopping(next_timeout)))
            {
                DEEPBLUE_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
                break;
            }
