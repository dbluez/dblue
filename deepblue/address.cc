...
namespace deepblue
{
                         ...
                         int family, int type, int protocol)
    {
        ...
        if (error)
        {
            DEEPBLUE_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
        ...
        while (next)
        {
            ...
            // DEEPBLUE_LOG_DEBUG(g_logger) << "family:" << next->ai_family << ", sock type:" << next->ai_socktype;
            next = next->ai_next;
        }
                                        ...
                                        int family)
    {
        ...
        {
            DEEPBLUE_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                            " err="
        ...
        {
            DEEPBLUE_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port)
    {
        ...
        if (error)
        {
            DEEPBLUE_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                                         << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            DEEPBLUE_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            DEEPBLUE_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                                         << port << ") rt=" << result << " errno=" << errno
                                         << " errstr=" << strerror(errno);
            return nullptr;
        }
